...
def _write_row(ws, row: int, item: dict):
    ...
    if _status(item) == "LOW":
        ws.cell(row=row, column=10).font = Font(bold=True, color="B91C1C")


def _autosize(ws):
    widths = {1: 46, 2: 18, 3: 22, 4: 12, 5: 10, 6: 8, 7: 14, 8: 16, 9: 18, 10: 10}
...
def _write_summary_sheet(ws, inv: dict):
    ...
    headers = ["Distributor / Warehouse", "SKU Count", "Units on Hand", "Inventory Value", "Low-Stock SKUs"]
    for col, label in enumerate(headers, start=1):
    ...
    for dist in sorted(dist_groups):
        ...
        ws.cell(row=row, column=5, value=d_low).font = SECTION_FONT
        grand_sku += d_sku
        grand_qty += d_qty
        grand_val += d_val
        grand_low += d_low
    ...
    for col, w in {1: 34, 2: 12, 3: 15, 4: 18, 5: 16}.items():
        ws.column_dimensions[get_column_letter(col)].width = w
